...
print("vector-db cli.py CHROMADB_HOST:", CHROMADB_HOST)
print("vector-db cli.py CHROMADB_PORT:", CHROMADB_PORT)
...
def download():
	source_blob_name = "smarteats_data/nutrition-books-embeddings.zip"  # Path to the file in the bucket

	# Initialize the Google Cloud Storage client
	storage_client = storage.Client()

    # Set up the bucket and blob
	...
	local_file_path = os.path.join(OUTPUT_FOLDER, os.path.basename(source_blob_name))

    # Download the blob to a local file
	with open(local_file_path, "wb") as f:
		with blob.open("rb") as blob_stream:
			for chunk in iter(lambda: blob_stream.read(8952), b''):
				f.write(chunk)

	if zipfile.is_zipfile(local_file_path):
		with zipfile.ZipFile(local_file_path, 'r') as zfile:
			zfile.extractall(OUTPUT_FOLDER)
			print(f"Extracted {local_file_path} to {OUTPUT_FOLDER}")

        # Locate and move the .jsonl file
		extracted_dir = os.path.join(OUTPUT_FOLDER, "nutrition-books-embeddings")
        
        # Find the .jsonl files in the extracted directory
        ...
        # Optionally, remove the now-empty nutrition-books-embeddings director
...
def chat(method="char-split", query_text=None):  # change it to allows chat query
    ...
    # Get a collection object from an existing collection, by name. If it doesn't exist, create it.
    collection_name = f"recursive-split-collection"
