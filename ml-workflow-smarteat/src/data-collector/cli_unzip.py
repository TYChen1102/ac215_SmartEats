...
def main(args=None):
    ...
    if args.search:
        ...
        for search_term in search_term_list:
            ...
            # Upload downloaded images to bucket folder
            search_folder = os.path.join(raw_folder, search_term)
            for file_path in glob(search_folder + "/*"):
                if os.path.isfile(file_path):
                    blob_name = f"{search_term}/{os.path.basename(file_path)}"
                    blob = bucket.blob(blob_name)
                    print(f"Uploading image: {blob_name}")
                    blob.upload_from_filename(file_path)

            print(f"Finished uploading images for '{search_term}'.")
